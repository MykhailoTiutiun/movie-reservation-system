...
SELECT 1, 'IMAX', 'Large format screen with high-resolution visuals and enhanced sound'
WHERE NOT EXISTS (SELECT 1 FROM auditoriums WHERE id = 1);

INSERT INTO auditoriums(id, name, description)
SELECT 2, '4DX', 'Immersive experience with motion seats and environmental effects'
WHERE NOT EXISTS (SELECT 1 FROM auditoriums WHERE id = 2);

INSERT INTO auditoriums(id, name, description)
SELECT 3, '3D', 'Three-dimensional projection for a depth-filled viewing experience'
