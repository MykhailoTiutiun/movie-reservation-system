...
public class ShowtimeResponseMapper {
    ...
    public ShowtimeResponse toShowtimeResponse(Showtime showtime){
        return new ShowtimeResponse(showtime.getId(), showtime.getDate().toString(), showtime.getStartTime().toString(), showtime.getEndTime().toString(), showtime.getAuditoriumId(), showtime.getMovieId());
    }
}
